...
class Message(object):
    def __init__(self, filename):
        ...
        try:
            ...
            emsg = email.message_from_string(self.data)
            self.top = '\r\n'.join(['%s: %s' % (k, v) for k,v in emsg._headers])
            self.bot = self.data.split('\r\n')[1:]
            # self.top, bot = self.data.split("\r\n\r\n", 1)
            # self.bot = bot.split("\r\n")
        finally:
            msg.close()
...
if __name__ == "__main__":
    # args = (None, '110', r'Resource/mail_samples/normal_attachment_txt.eml')
    args = sys.argv
    if len(args) != 3:
        print "USAGE: [<host>:]<port> <path_to_message_file>"
    else:
        _, port, filename = args
        if ":" in port:
